...
@section('seo_support')
    ...
    @if($service->image != null)
        <meta property="og:image" content="{{route('posts.detailEn',$service->image)}}">
    @else
        <meta property="og:image" content="{{asset_theme ('img/logo.png')}}">
    @endif
    <meta property="og:url" content="{{route('detail_servicesEn', ['slug' => $service->slug])}}">
    <meta name="twitter:title" content="{{$service->title}}">
    @if($service->meta_description != null)
        <meta name="twitter:description" content="{{$service->meta_description}}">
    @endif
    @if($service->image != null)
        <meta name="twitter:image" content="{{route('posts.detailEn',$service->image)}}">
    @else
    ...
    <div class="container">
        <div class="row title">
            ...
            <div class="page-breadcrumb">
                <a href="{{route('homeEn')}}">{{__('Home')}}</a>/<span>{{$service->title}}</span>
            </div>
        </div>
    </div>
</section>
        ...
        <div class="row">
            ...
            <div class="col-md-12 blog-post-hr">
                <div class="blog-post mb-30">
                    <div class="post-meta"><span> <a>{{__('Solution')}}</a>,</span> <span>{{ \Carbon\Carbon::parse($service->created_at)->format('d/m/Y')}} </span></div>
