...
@section('seo_support')
    ...
    @if($partner->image != null)
        <meta property="og:image" content="{{route('posts.detailEn',$partner->image)}}">
    @else
        <meta property="og:image" content="{{asset_theme ('img/logo.png')}}">
    @endif
    <meta property="og:url" content="{{route('partners.detailEn',$partner->slug)}}">
    <meta name="twitter:title" content="{{$partner->name}}">
    @if($partner->meta_description != null)
        <meta name="twitter:description" content="{{$partner->meta_description}}">
    @endif
    @if($partner->image != null)
        <meta name="twitter:image" content="{{route('posts.detailEn',$partner->image)}}">
    @else
...
@section('content')
    <section class="inner-intro overlay-dark bg-img1 light-color">
        <div class="container">
            <div class="row title">
                <h2 class="h2">{{$partner->name}} - {{__('Partner')}}</h2>
                <div class="page-breadcrumb">
                    <a href="{{route('homeEn')}}">{{__('Home')}}</a>/<span>{{$partner->name}}</span>
                </div>
            </div>
        </div>
    </section>
