...
@section('seo_support')
    <title>{{get_site_setting('products-list-title-en') }}</title>
    <meta name="description" content="{{get_site_setting('products-list-description-en') }}">
    <meta name="keywords" content="{{get_site_setting('products-list-keywords-en') }}">
    <meta property="og:title" content="{{get_site_setting('products-list-title-en') }}">
    <meta property="og:description" content="{{get_site_setting('products-list-description-en') }}">
    @if($list[0] == null || $list[0]['image'] == null)
        <meta property="og:image" content="{{asset_theme ('img/logo.png')}}">
    @else
        <meta property="og:image" content="{{route('posts.detailEn',$list[0]['images']->first()['image'])}}">
    @endif
    <meta property="og:url" content="{{route('productsEn')}}">

    <meta name="twitter:title" content="{{get_site_setting('products-list-title-en') }}">
    <meta name="twitter:description" content="{{get_site_setting('products-list-description-en') }}">
    <meta name="twitter:card" content="summary_large_image">
    @if($list[0] == null || $list[0]['image'] == null)
        <meta property="twitter:image" content="{{asset_theme ('img/logo.png')}}">
    @else
        <meta name="twitter:image" content="{{route('posts.detailEn',$list[0]['images']->first()['image'])}}">
    @endif
@endsection
@section('after_css')
                                    ...
                                    <div class="col-md-11 nf-item spacing-grid">
                                        <div class="blog-post">
                                            <div class="post-media">
                                                <blockquote>{{__('The product is updating!')}}</blockquote>
                                            </div>
                                        </div>
                                    </div>
                                @endif
