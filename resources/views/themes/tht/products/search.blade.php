...
@section('seo_support')
    <title>{{get_site_setting('products-search-title') }}</title>
    <meta name="description" content="{{get_site_setting('products-search-description') }}">
    <meta name="keywords" content="{{get_site_setting('products-search-keywords') }}">
    <meta property="og:title" content="{{get_site_setting('products-search-title') }}">
    <meta property="og:description" content="{{get_site_setting('products-search-description') }}">
    @if(($product[0]) !== null)
        <meta property="og:image" content="{{route('posts.detail',$product[0]['images']->first()['image'])}}">
    @else
        <meta property="og:image" content="{{asset_theme ('img/logo.png')}}">
    @endif
    <meta property="og:url" content="{{route('products.search')}}">

    <meta name="twitter:title" content="{{get_site_setting('products-search-title') }}">
    <meta name="twitter:description" content="{{get_site_setting('products-search-description') }}">
    <meta name="twitter:card" content="summary_large_image">
    @if(($product[0]) !== null)
        <meta name="twitter:image" content="{{route('posts.detail',$product[0]['images']->first()['image'])}}">
    @else
            ...
            @foreach ($product as $item)
                <div class="nf-item col-md-3 col-sm-6 col-xs-12 mb-30">
                    <div class="item-box">
                        ...
                        <a href="{{route('products.detail',$item->slug)}}"> <h6 class="shop-item-name">{{$item->name ??''}}</h6> </a>
