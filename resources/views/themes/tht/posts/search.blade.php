...
@section('seo_support')
    <title>{{get_site_setting('posts-search-title') }}</title>
    <meta name="description" content="{{get_site_setting('posts-search-description') }}">
    <meta name="keywords" content="{{get_site_setting('posts-search-keywords') }}">
    <meta property="og:title" content="{{get_site_setting('posts-search-title') }}">
    <meta property="og:description" content="{{get_site_setting('posts-search-description') }}">
    @if(($post[0]) !== null)
    ...
    <meta name="twitter:title" content="{{get_site_setting('posts-search-title') }}">
    <meta name="twitter:description" content="{{get_site_setting('posts-search-description') }}">
                    ...
                    <div class="container">
                       <!-- Post Item -->
                        <div class="row container-masonry">
                                @if($count>0)
                                <p>Tìm thấy {{$count}} kết quả <i><b>{{$key}}</b></i> </p>
                                @else
                                <p>Không tìm thấy kết quả với  <i><b>{{$key}}</b></i></p>
                                @endif
                                ...
                                <div class="col-md-4 col-sm-6 col-xs-12 nf-item spacing-grid">
                                    <div class="blog-post">
                                        ...
                                        <div class="post-header">
                                            <h5><a href="{{route('posts.detail', ['slug' => $item->slug])}}">{{ $item->title}}</a></h5>
                                        </div>
