...
//            $rs['catalog_id'] = $user_id;
   ...
   public function getProduct(){
        ...
        $model_type = $this->getModel('ProductType')->where('locale','=','vi');
        $model_cate = $this->getModel('ProductCategory')->where('locale','=','vi');
...
//        }
       $cate=$model_cate->orderBy('order')->get();
            return  view('themes.tht.products.list',compact('list','cate','partners'));
   ...
   public function detail($slug){
    ...
    $product=$this->getModel("Product")->where('locale','=','vi')->where('slug',$slug)->first();
    if(empty($product)){
    ...
    $productImages=$this->getModel("ProductImage")::where('product_id',$product->id)->where('locale','=','vi')->get();
    $productImageFist=$this->getModel("ProductImage")::where('product_id',$product->id)->where('locale','=','vi')->first();
    // dd($product_news);
    $catalogs=$product->catalogs;
    $typename=$type->name;
        return  view('themes.tht.products.detail',compact('product','catalogs','cate_with_type','type','cate','productImages','product_other','product_news','partners','typename','productImageFist'));
   }
   function getListType($slug){
    ...
    // }
           return  view('themes.tht.products.filter',compact('list_type','count','product_type','cate','product_type','cate_with_type','partners'));
...
//       dd($list_type);
       ...
       $partners= $this->getModel('Partner')->where('locale','=','vi')->orderBy('order')->active()->get();
       return  view('themes.tht.products.cate',compact('product_cate','count','list_type','cate','id_cate','partners','slug_cate'));
