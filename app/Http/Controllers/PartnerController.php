...
class PartnerController extends Controller
{
    ...
    public function getListPartner()
    {
        ...
        //get list blog
        $list = $this->getModel('Partner')->where('locale','=','vi')->orderBy('order')->get();
        //check null exception
            return view('themes.tht.partners.list', ['list' => $list]);

    }
    public function detail($slug){
        $partner=$this->getModel("Partner")->where('locale','=','vi')->where('slug',$slug)->first();
        $partners= $this->getModel('Partner')->where('locale','=','vi')->orderBy('order')->active()->get();
        if(empty($partner))
        {
            return redirect()->back();
        }
        $partner_more=$this->getModel("Partner")->where('locale','=','vi')->where('id','!=',$partner->id)->get();

            return  view('themes.tht.partners.detail',compact('partner','partners','partner_more'));
    }
}
