...
class PostController extends Controller
{
    ...
    public function detail(Request $request, $slug)
    {
        ...
        if ($post && $post->type == 'post') {
            ...
            $categories = $this->getModel('Category')->where('locale','=','vi')->orderBy('created_at')->take(5)->get();
            ...
            $relatedPosts = $this->getModel('Post')->where('locale','=','vi')->active()->post()
            ...
            */
            ...
            $partners= $this->getModel('Partner')->where('locale','=','vi')->orderBy('order')->get();
            return view('themes.tht.pages.detail', compact('post','partners'));
        }
    }
    ...
    public function getListPost()
    {
        ...
        Carbon::now('Asia/Ho_Chi_Minh'); //set time VN
        //get list blog
        $list = $this->getModel('Post')->where('locale','=','vi')->orderBy('created_at', 'DESC')->post()->active()->paginate(12);
        //check null exception
        $cate = $this->getModel('Category')->where('locale','=','vi')->orderBy('created_at', 'DESC')->get();
        $partners= $this->getModel('Partner')->where('locale','=','vi')->orderBy('order')->get();
        $count = $list->total();
//        if ($count == 0) {
//            return redirect('/');
//        }
            return view('themes.tht.posts.list', ['list' => $list, 'partners' => $partners ,'cate'=>$cate])->with('notifi');
    }

    public function listCate($slug)
    {
        // set Model
        $model_cate = $this->getModel("Category")->where('locale','=','vi');
        $model_post = $this->getModel("Post")->where('locale','=','vi');
        ...
        $catelist = $this->getModel('Category')->where('locale','=','vi')->orderBy('created_at', 'DESC')->get();
...
//        }
//        dd($id_post[0]);
            return view('themes.tht.posts.category', compact( 'id_post', 'cate_name', 'list', 'count', 'partners', 'cate', 'cate_id', 'catelist', 'cate_slug', 'cate_name'));
}
