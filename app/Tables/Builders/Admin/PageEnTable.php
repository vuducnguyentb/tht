...
class PageEnTable extends Table
{
    protected $templatePath = __DIR__.'/../../Templates/Admin/pages.json';

    public function query() {
        if(env('APP_DEBUG', false) == true) {
            Log::info("PageEnTable->query");
        }
        ...
        */
        return Post::join('users','posts.author_id','=','users.id')
            ->select(
                ...
                'posts.title',
                'posts.created_at')->where('posts.type', '=', 'page')->where('posts.locale','=','en');
    }
}
